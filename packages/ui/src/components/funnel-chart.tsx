...
/**
 * Animated funnel chart, vendored from a shadcn registry component at JV's
 * request and adapted to this repo: shared `cn`, design-system tokens instead
 * of the registry's (`--chart-grid`, `--color-muted` do not exist here), and
 * no local clsx/tailwind-merge copies.
 *
 * `useEffect` throughout is the vendored animation plumbing (springs reacting
 * to hover/mount and a ResizeObserver) — external subscriptions, which is the
 * sanctioned use.
 */
...
export function FunnelChart({
	...
	const bandColor = gridCfg.bandColor ?? "var(--muted)";
	const showGridLines = gridEnabled && (gridCfg.lines ?? true);
	const gridLineColor = gridCfg.lineColor ?? "var(--border)";
